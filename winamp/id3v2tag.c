 ...
 *
 * $Id: id3v2tag.c,v 1.14 2001/10/07 14:55:59 menno Exp $
 */
...
void FillID3List(HWND hwndDlg, HWND hwndList, char *filename)
{
    ...
    if ((tag = ID3Tag_New()) != NULL)
    {
        ...
        if (numFrames)
            SendMessage(GetDlgItem(hwndDlg, IDC_ID3V2TAG), BM_SETCHECK, BST_CHECKED, 0);
        else
...
void List_AddFrame(HWND hwndApp, HWND hwndList)
{
    ...
    result = DialogBox(mod.hDllInstance, MAKEINTRESOURCE(IDD_ADDFRAME),
        hwndApp, AddFrameProc);

    if (LOWORD(result) == IDOK)
    {
        SendMessage(GetDlgItem(hwndApp, IDC_ID3V2TAG), BM_SETCHECK, BST_CHECKED, 0);
        EnableWindow(GetDlgItem(hwndApp, IDC_ID3V2TAG), TRUE);
    }
...
BOOL CALLBACK AddStandardProc(HWND hwndDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
        {
            ...
            if (added > 0)
            {
                SendMessage(GetDlgItem(m_hwndDlg, IDC_ID3V2TAG), BM_SETCHECK, BST_CHECKED, 0);
                EnableWindow(GetDlgItem(m_hwndDlg, IDC_ID3V2TAG), TRUE);
            }
        }
        case IDCANCEL:
            EndDialog(hwndDlg, wParam);
...
void List_AddStandardFrames(HWND hwndApp, HWND hwndList)
{
    ...
    DialogBox(mod.hDllInstance, MAKEINTRESOURCE(IDD_ADDSTANDARD),
...
void List_EditData(HWND hwndApp, HWND hwndList, char *filename)
{
    ...
    if (index != -1)
    {
        ...
        if (ListView_GetItem(hwndList, &lvi) == TRUE)
        {
            ...
            DialogBox(mod.hDllInstance, MAKEINTRESOURCE(IDD_EDITTEXTFRAME),
...
void List_DeleteSelected(HWND hwndApp, HWND hwndList)
{
    ...
    if (items == 0)
    {
        SendMessage(GetDlgItem(m_hwndDlg, IDC_ID3V2TAG), BM_SETCHECK, BST_UNCHECKED, 0);
        EnableWindow(GetDlgItem(m_hwndDlg, IDC_ID3V2TAG), FALSE);
    }
}
...
void List_SaveID3(HWND hwndApp, HWND hwndList, char *filename)
{
    ...
    if ((tag = ID3Tag_New()) != NULL)
    {
        ...
        if (SendMessage(GetDlgItem(m_hwndDlg, IDC_ID3V2TAG), BM_GETCHECK, 0, 0) == BST_UNCHECKED)
        {
            /* No frames saved */
            ID3Tag_Delete(tag);
            return;
