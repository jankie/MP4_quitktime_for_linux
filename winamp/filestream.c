 ...
 *
 * $Id: filestream.c,v 1.14 2001/09/24 07:31:58 menno Exp $
 */
...
void seek_filestream(FILE_STREAM *fs, unsigned long offset, int mode)
{
	...
	{
		mode = FILE_BEGIN;
		offset += tell_filestream(fs);
	}

	fs->file_offset = SetFilePointer(fs->stream, offset, NULL, mode);
...
int http_file_open(char *url, FILE_STREAM *fs)
{
    SOCKET sck;
    SOCKADDR_IN host;
    char server[1024], file[1024], request[1024], *temp, *tmpfile;
    ...
    wsprintf(request, "GET %s HTTP/1.1\r\n\r\n", tmpfile);
    ...
    for(j=0; j < 20; j++)
    {
        if(StringComp(temp, "Content-Length: ", 16) == 0)
        {
